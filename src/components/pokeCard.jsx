...
export default function PokeCard(props) {
    ...
    const { name, height, ablities, stats, types, moves, sprites } = data || {};
    ...
    useEffect(() => {
        // if loading, exit the logic 
        if (loading || !localStorage) {
            return;
        }

        // check if the selected pokemon information is availabele in cache
        // 1. define the cache
        let cache = {};
            ...
            <div className="img-container">
                {
                    imgList.map((spriteUrl, spriteIndex) => {
                        ...
                        return (
                            <img src={imgUrl} alt={`${name}-img-${spriteUrl}`} key={spriteIndex}/>
                        )
                    })
                }
            </div>
            ...
            <div className="pokemon-move-grid">
                {
                    moves.map((moveObj, moveIndex) => {
                        return (
                            <button className="button-card" key={moveIndex} onClick={() => {
